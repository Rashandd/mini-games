"""Async SE command processor — dict mutations unchanged, async chain for save."""
import json

from social_empires.sessions import session, save_session
from social_empires.get_game_config import (
    get_game_config, get_level_from_xp, get_name_from_item_id,
    get_attribute_from_mission_id, get_xp_from_level,
    get_attribute_from_item_id, get_item_from_subcat_functional,
)
from social_empires.constants import Constant
from social_empires.engine import apply_cost, apply_collect, apply_collect_xp, timestamp_now
...
def do_command(USERID, cmd, args):
    ...
    if cmd == Constant.CMD_GAME_STATUS:
        ...
        frame = args[3]
        town_id = args[4]
        bool_dont_modify_resources = bool(args[5])
        ...
        reward = int(get_attribute_from_mission_id(mission_id, "reward"))
        ...
        map["xp"] = max(min_expected_xp, current_xp)
        ...
        collected_at_timestamp = timestamp_now()
        level = 0
        items += [[item_id, x, y, orientation, collected_at_timestamp, level]]
        save["privateState"]["gifts"][item_id] -= 1
    ...
    elif cmd == Constant.CMD_DESACTIVATE_MONSTER:
